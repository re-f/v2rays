...
# echo "https://download.fastgit.org/v2fly/v2ray-core/releases/download/${TAG}/${V2RAY_FILE}"
wget -O ${PWD}/v2ray.zip https://download.fastgit.org/v2fly/v2ray-core/releases/download/${TAG}/${V2RAY_FILE} > /dev/null 2>&1
wget -O ${PWD}/v2ray.zip.dgst https://download.fastgit.org/v2fly/v2ray-core/releases/download/${TAG}/${DGST_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
